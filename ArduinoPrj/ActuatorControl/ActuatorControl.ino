/*
 * THIS IS RUDDER and THRUSTER MOTOR
 * Programmer's note: I use #warning to mark the TODO list
 *
 * Receive input from Raspberry Pi and send output to Rudder and Thruster motor controller.
 */
...
//#undef MEGA2560
#define GEMSMOTOR
//#define TECNADYNEMOTORTHRUSTER
//#undef GEMSMOTOR
#undef TECNADYNEMOTORTHRUSTER
...
bool afterCHECKSUM[]={false,false,false};
...
void setDesiredAction(){
    ...
    switch(cmd) {
      ...
      case 0x04:
        ...
        break;
      default:
        Serial.println("Received the wrong command");
        break;
    }
...
{
  if (motor_address == motorcontrol_rs485_address[2])
  {
    #ifdef TECNADYNEMOTORTHRUSTER
    if(afterCHECKSUM[motor_address-0x60]){
      uint8_t motor_adir = dta_buf[0] ? -dta_buf[1] : dta_buf[1];
      setSpeedPWM(motor_address, motor_adir, motor_preferred_pwm);
      motor_address = 0x00;
      cmdFlag = 999;//clr cmdFlag
      read_state = HEADERPT1;
      afterCHECKSUM[motor_address-0x60] = false
    }
    #endif
    #ifdef GEMSMOTOR
    if(afterCHECKSUM[motor_address-0x60]){
//       Serial.println("\nSpeed is");
//       Serial.print(dta_buf[1],HEX);
      bool bstartstopflag = (dta_buf[1]>0)? true : false;
//       Serial.println("\nbstartstopflag is");
...
//       Serial.println();
      ...
      if (bstartstopflag)
      {
        ...
        if(motor_adir==0)
          ...
          digitalWrite(pinThuFR, LOW);//LOW is CLOSE, reverse
      }
  ...
  else if(motor_address == motorcontrol_rs485_address[0])
  {
...
void setup()
{
  ...
  uint8_t flying_bcc = 0x00;
  read_state = HEADERPT1;
  afterCHECKSUM[0] = false;afterCHECKSUM[1] = false;afterCHECKSUM[2] = false;
...
/*    Serial.print("Got byte: ");
    ...
    Serial.print(read_state, HEX);
    Serial.print("\tflying_bcc: ");
    Serial.print(flying_bcc, HEX);
    Serial.print("\n");*/
    ...
    switch(read_state) {
      ...
      case CHECKSUM:
        ...
        // Send strings to motor controller.
          setDesiredAction();
          afterCHECKSUM[motor_address-0x60] = true;
  ...
  *****************************************************************************************************************************/
  ...
  /*
  *check the encoder reach the destination, position control to stop the motor by encoder feedback
  *****************************************************************************************************************************/
  if(( motor_address == motorcontrol_rs485_address[0] )&& (motor_state == 1))
  {
  ...
  *****************************************************************************************************************************/
  #if 0
 // no need for it since it is drown in the water
